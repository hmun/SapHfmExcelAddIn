...
Public Class TSAP_OsMdData
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAP_ZFAGL_UPD_ZOSMSUD As SAP_ZFAGL_UPD_ZOSMSUD, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        ...
        If pFunction = "Update" Then
            pSAP_ZFAGL_UPD_ZOSMSUD.getMeta_Update(aParamDic, aStrucDic)
        End If
    End Sub
    ...
    Public Function fillData(pData As TData) As Boolean
        ...
        For Each aKvB In pData.aTDataDic
            ...
            For Each aTStrRec In aTDataRec.aTDataRecCol
                If valid_Table_Field(aTStrRec) Then
                    If aTStrRec.Strucname = "I_T_DATA" Then
                        aNewTStrRec = New SAPCommon.TStrRec("I_T_DATA", "IOBJNM", aTStrRec.Fieldname, aTStrRec.Currency, aTStrRec.Format)
                        aDataDic.addValue(CStr(aCnt) & "-" & aTStrRec.Fieldname, aNewTStrRec)
                        aNewTStrRec = New SAPCommon.TStrRec("I_T_DATA", "RECORD_NO", CStr(aCnt), aTStrRec.Currency, aTStrRec.Format)
                        aDataDic.addValue(CStr(aCnt) & "-" & aTStrRec.Fieldname, aNewTStrRec)
                        aNewTStrRec = New SAPCommon.TStrRec("I_T_DATA", "VALUE", aTStrRec.Value, aTStrRec.Currency, aTStrRec.Format)
                        aDataDic.addValue(CStr(aCnt) & "-" & aTStrRec.Fieldname, aNewTStrRec)
                        aNewTStrRec = New SAPCommon.TStrRec("I_T_ATTRIBUTES", "IOBJNM", aTStrRec.Fieldname, aTStrRec.Currency, aTStrRec.Format)
                        aDataDic.addValue("1" & "-" & aNewTStrRec.Value, aNewTStrRec)
                    End If
                End If
            Next
            aCnt += 1
